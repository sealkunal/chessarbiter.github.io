 ...
 * Indexes all 12 chapter JSON files and searches at runtime.
 */
...
// ─── Build index from all chapter JSON files ──────────────────────────────
async function buildIndex() {
  ...
  // Flatten all articles into the document store
  docStore = {};
  ...
  resultsWrap.innerHTML = `<p class="no-results" style="color:var(--muted)">
    Search index ready (${Object.keys(docStore).length} articles across ${index.chapters.length} chapters)
...
function doSearch() {
  ...
  catch { results = lunrIndex.search(query);        }
  ...
  resultsWrap.innerHTML = `
    ...
    ${results.map(r => {
      ...
      const snip = doc.content.length > 240 ? doc.content.slice(0, 240) + '…' : doc.content;
      return `
...
// Auto-fill from URL param e.g. search.html?q=stalemate
